...
class TestBigInt extends haxe.unit.TestCase {
	...
	public function testFromBaseString() {
		...
		// errors
		assertTrue(try {
			BigInt.fromBaseString("0x ff", 16);
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("1", "");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("1", "1");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("1", "1");
		...
		assertTrue(try {
			BigInt.fromBaseString("a", "123");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("10", 5, "0123");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("10", "0 123");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("10", "0 12 3");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("10", "0  12-3");
		...
		assertTrue(try {
			BigInt.fromBaseString("10", "0 1--2-3");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBaseString("10", "-0123");
			false;
		} catch (e:Dynamic) true);
	}
	...
	public function testToBaseString() {
		...
		// errors
		assertTrue(try {
			(10 : BigInt).toBaseString("");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString("a");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString(0, "a");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString(1, "a");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString(4, "abc");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString("-abc");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString("-ab--c");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString("a bc");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			(10 : BigInt).toBaseString("a bc  ");
	...
	public function testFromHexString() {
		...
		assertTrue(try {
			BigInt.fromHexString(" 0x ff ");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromHexString("0xff ");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromHexString("-0xff ");
	...
	public function testFromBinaryString() {
		...
		// errors
		assertTrue(try {
			BigInt.fromBinaryString("0b");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBinaryString("0b101");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBinaryString("-0b101");
		...
		assertTrue(try {
			BigInt.fromBinaryString("-10-1");
			false;
		} catch (e:Dynamic) true);
		assertTrue(try {
			BigInt.fromBinaryString("2");
			false;
		} catch (e:Dynamic) true);
	}
	...
	public function testMultiplication() {
		...
		assertEquals((("1761804960632708497166197429" : BigInt) * ("170141183460469231731687303715884105709" : BigInt)).toString(),
			"299755581028574428885627854637518938878751711898852594203800022161");
	}
	...
	public function testModulo() {
		...
		assertEquals((("2938289887273578834588723405828340070292347195862712329756482349" : BigInt) % ("2938289887273578834588723405828340070292347195862712329756482300" : BigInt))
			.toInt(),
			49);
		assertEquals((("2938289887273578834588723405828340070292347195862712329756482349" : BigInt) % ("-2938289887273578834588723405828340070292347195862712329756482300" : BigInt))
			.toInt(),
		...
		assertEquals((BigInt.fromHexString('D5D4921855F54CC4D0EC0E0CBEF8B582A11E959D54243145088DEF36C75886EEBAE84160B64E011F6B9AD301F1D3466E247FC2ACF7B5DF1798D72918C459CF0') % BigInt.fromHexString('7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFED'))
			.toString(),
	...
	public function testPow() {
		...
		assertTrue((252 : BigInt)
			.pow(124) == "");
	...
	public function testPowMod() {
		...
		assertTrue(("4139543763576876256978864589462346353452" : BigInt).powMod("1322448978334799562386783579042534564354",
			"19096727818566719527493245743456") == "17450867125893875188144890320704");
		assertTrue(("-4139543763576876256978864589462346353452" : BigInt).powMod("1322448978334799562386783579042534564354",
			"19096727818566719527493245743456") == "17450867125893875188144890320704");
	...
	public function testBitwiseOperations() {
		for (i in -256...257)
			assertEquals((~(i : BigInt)).toString(), '${~i}');
		for (i in -0x7ffffffe... -0x7fffff00)
			assertEquals((~(i : BigInt)).toString(), '${~i}');
		for (i in 0x7fffff00...0x7fffffff)
			assertEquals((~(i : BigInt)).toString(), '${~i}');
		...
		assertTrue((("0b 1101" : BigInt) & ("0b 11111111 11111111 11111111 11111111 11111111 11111111 11111111 11111111 11111111 11111100 11100101 11001110 10111001 11010010" : BigInt))
			.isZero);
		...
		assertTrue((("0b 11001011100011110110101110101000100111001110011111101111101111" : BigInt) ^ ("0b 11001011100011110110101110101000100111001110011111101111101111" : BigInt))
			.isZero);
		assertTrue((("0b 11001011100011110110101110101000100111001110011111101111101110" : BigInt) ^ ("0b 11001011100011110110101110101000100111001110011111101111101111" : BigInt))
			.chunksLength == 1);
	}
