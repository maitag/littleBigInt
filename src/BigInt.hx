...
abstract BigInt(LittleIntChunks) from LittleIntChunks {
	...
	inline function get(i:Int):LittleInt
		return this.get(i);

	inline function set(i:Int, v:LittleInt)
		this.set(i, v);

	inline function push(v:LittleInt)
		this.push(v);

	inline function unshift(v:LittleInt)
		this.unshift(v);

	inline function splitHigh(e:Int):BigInt
		return this.splitHigh(e);

	inline function splitLow(e:Int):BigInt
		return this.splitLow(e);

	inline function truncateZeroChunks(remove:Bool)
		this.truncateZeroChunks(remove);

	inline function setNegative():BigInt {
		this.isNegative = true;
		return this;
	}

	inline function setPositive():BigInt {
		this.isNegative = false;
		return this;
	}

	inline function copy():BigInt
		return new BigInt(this.copy());

	inline function negCopy():BigInt
		return new BigInt(this.negCopy());

	inline function clone():BigInt
		return new BigInt(this.clone());

	inline function negClone():BigInt
		return new BigInt(this.negClone());
	...
	**/
	...
	inline function get_isPositive():Bool
		return !this.isNegative;
	...
	**/
	...
	inline function get_chunksLength():Int
	...
	**/
	public function toBinaryString(spacing:Int = 0, leadingZeros:Bool = true):String {
		if (this == null)
			return (leadingZeros && spacing > 0) ? LittleIntChunks.getStringOfZeros(spacing) : "0";
	...
	**/
	public function toBaseString(base:Null<Int> = null, digitChars:Null<String> = null, spacing:Int = 0, leadingZeros:Bool = true):String {
		...
		if (this == null) {
			...
			if (digitChars != null) {
				if (base < 2)
					throw('Error, base $base need to be greater or equal 2');
				if (base > digitChars.length)
					throw('Error, base $base for string output is to great. Max value can be ${digitChars.length}');
	...
	**/
	@:op(A + B) function opAdd(b:BigInt):BigInt
		return _add(this, b);

	// @:op(A + B) @:commutative function opAddInt(b:Int):BigInt return _add(this, b);
	@:op(A + B) static function opAddInt(a:Int, b:BigInt):BigInt
		return _add(a, b); // haxe 3.4.4 compatible!
	...
	static inline function _add(a:BigInt, b:BigInt):BigInt {
		...
		if (b.isNegative)
			return __subtract(a, b.negClone()); // 3 + -2
	...
	/**
		Returns `a` minus `b`.
	**/
	@:op(A - B) function opSubtract(b:BigInt):BigInt
		return _subtract(this, b);

	// @:op(A - B) static function opSubtractInt(a:BigInt, b:Int):BigInt return _subtract(a, b);
	@:op(A - B) static function opIntSubtract(a:Int, b:BigInt):BigInt
	...
	static inline function _subtract(a:BigInt, b:BigInt):BigInt {
		if (a == null)
			return (b == null) ? null : b.negCopy();
		if (b == null)
			return a.copy();
		if (a.isNegative) {
			if (b.isNegative)
				return __subtract(b.negClone(), a.negClone()); // -3 - -2
			return __add(a, b).setNegative(); // -3 - 2
		}
		if (b.isNegative)
			return __add(a, b.negClone()); // 3 - -2
	...
	**/
	...
	function opMulticplicate(b:BigInt):BigInt {
		...
		if (isNegative != b.isNegative)
			return mul(this, b).setNegative();
		return mul(this, b).setPositive();
	}

	// @:op(A * B) @:commutative function opMulticplicateInt(b:Int):BigInt return opMulticplicate(b);
	@:op(A * B) static inline function opMulticplicateInt(a:Int, b:BigInt):BigInt
		return b.opMulticplicate(a); // haxe 3.4.4 compatible!
	...
	**/
	...
	@:op(A / B) static function opIntDiv(a:Int, b:BigInt):BigInt
	...
	**/
	...
	@:op(A % B) static function opIntModulo(a:Int, b:BigInt):BigInt
	...
	**/
	static public inline function divMod(dividend:BigInt, divisor:BigInt):{quotient:BigInt, remainder:BigInt} {
		if (divisor == null)
			throw("Error '/', divisor can't be 0");
		if (dividend == null)
			return {quotient: null, remainder: null}; // handle null
		if (divisor == 1)
			return {quotient: dividend.copy(), remainder: null}; // handle dividing by 1
		if (dividend == divisor)
			return {quotient: 1, remainder: null}; // handle equal
	...
	static inline function _divMod(a:BigInt, b:BigInt):{quotient:BigInt, remainder:BigInt} {
		if (b.length <= 2) {
			...
			if (b.length == 1) {
				if (b.get(0) == 1)
					return {quotient: a, remainder: null};
	...
	static inline function divFast(a:BigInt, v:LittleInt):BigInt {
		...
		if (a == v)
			return 1; // handle equal
		if (v == 1)
			return a.copy(); // handle dividing by 1
		if (a.length <= 2)
			return Std.int(a.toInt() / v); // TODO: optimized toInt() without bitsize-check!
	...
	**/
	public function pow(exponent:BigInt):BigInt {
		if (exponent < 0)
			throw("Error 'powMod', exponent can't be negative");
		if (exponent == null)
			return 1;
		if (this == null)
			return null;
		...
		if (length == 1 && get(0) == 1)
			return 1;
		if (exponent.length == 1 && exponent.get(0) == 1)
			return this.copy();
		...
		for (i in 0...exponent.length) {
			...
			if (i == exponent.length - 1)
				maxBits = 1 << IntUtil.bitsize(e);
			else
				maxBits = LittleIntChunks.UPPESTBIT;
			while (bit < maxBits) {
				if (bit & e != 0)
					result = result * base;
				base = base * base;
	...
	**/
	public function powMod(exponent:BigInt, modulus:BigInt):BigInt {
		if (exponent < 0)
			throw("Error 'powMod', exponent can't be negative");
		if (modulus == null)
			throw("Error 'powMod', modulus can't be 0");
		if (modulus == 1)
			return null;
		...
		for (i in 0...exponent.length) {
			...
			while (bit < maxBits) {
				if (bit & e != 0)
					result = divMod(result * base, modulus).remainder;
	...
	**/
	...
	@:op(A > B) static inline function intGreater(a:Int, b:BigInt):Bool
		return b.lesser(a);
	...
	**/
	...
	@:op(A >= B) static inline function intGreaterOrEqual(a:Int, b:BigInt):Bool
		return b.lesserOrEqual(a);
	...
	**/
	...
	@:op(A < B) static inline function intLesser(a:Int, b:BigInt):Bool
		return b.greater(a);
	...
	**/
	...
	@:op(A <= B) static inline function intLesserOrEqual(a:Int, b:BigInt):Bool
		return b.greaterOrEqual(a);
	...
	**/
	public function shiftOneBitLeft() {
		if (this != null) {
			...
			if (v & LittleIntChunks.UPPESTBIT > 0)
				push(1);
		}
	}
	...
	**/
	...
	function opShiftLeft(b:Int):BigInt {
		...
		if (b == 0)
			return this.copy();
		if (b < 0)
			throw("ERROR '<<', can't negative-shift-left a negative value");
	...
	**/
	...
	@:op(A & B) static inline function opANDInt(a:Int, b:BigInt):BigInt
		return b.opAND(a); // haxe 3.4.4 compatible!
	...
	**/
	...
	@:op(A | B) static inline function opOrInt(a:Int, b:BigInt):BigInt
		return b.opOR(a); // haxe 3.4.4 compatible!
	...
	**/
	...
	@:op(A ^ B) static inline function opXORInt(a:Int, b:BigInt):BigInt
		return b.opXOR(a); // haxe 3.4.4 compatible!
}
